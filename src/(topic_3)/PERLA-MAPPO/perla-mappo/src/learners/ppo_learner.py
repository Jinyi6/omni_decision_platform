...
class PPOLearner:
    ...
    def train_critic_sequential(self, critic, target_critic, batch, rewards, mask):
        ...
        indexing_matrix = th.tensor(indexing_matrix, device=batch.device).unsqueeze(0)
        indexing_matrix = indexing_matrix.repeat(self.args.n_samples, num_envs, num_timesteps, 1, 1)
        ...
        with th.no_grad():
            ...
            for t in range(batch.max_seq_length):
                ...
                raw_sampled_actions.append(cur_actions)
            sampled_actions = th.stack(raw_sampled_actions, dim=2)
            sampled_actions[th.unsqueeze(mask, 0).repeat(self.args.n_samples, 1, 1, 1) == 0] = 1.0
            sampled_actions = sampled_actions.unsqueeze(-1).repeat(1, 1, 1, self.n_agents, 1).view(self.args.n_samples, self.args.batch_size, batch.max_seq_length, self.n_agents, self.n_agents)
            sampled_actions = th.take_along_dim(sampled_actions, indexing_matrix, 4)
        mask = mask[:, :-1]
        ...
        # NEW
        # target_vals, _ = target_critic(batch, joint_action_minus_i=sampled_actions, type=1)
        # target_vals = target_vals[:, :, :-1]
        # target_vals = target_vals.squeeze(-1) 
        # V_target = th.mean(target_vals, dim=0) # -> (num_envs, num_timesteps, num_agents)
        ...
        for sampled_action in sampled_actions:
            ...
            target_vals, _ = target_critic(batch, joint_action_minus_i=sampled_action)
            target_vals = target_vals[:, :-1]
        ...
        # NEW
        # Sample E[V(s_t, a^{-i})]
        # vals, _ = critic(batch, joint_action_minus_i=sampled_actions)
        # vals = vals[:, :, :-1]
        # vals = vals.squeeze(-1)
        # V = th.mean(target_vals, dim=0) # -> (num_envs, num_timesteps, num_agents)
