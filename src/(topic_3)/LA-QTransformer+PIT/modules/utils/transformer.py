...
class MultiHeadAttention(nn.Module):
    def __init__(self, in_dim, model_dim, n_heads, dropout=0.0):
        ...
        self.fc_residual = nn.Linear(in_dim, model_dim)

        self.fc_k = nn.Linear(in_dim, self.dim_per_head * n_heads)
        self.fc_q = nn.Linear(in_dim, self.dim_per_head * n_heads)
        self.fc_v = nn.Linear(in_dim, self.dim_per_head * n_heads)
    ...
    def forward(self, k, v, q, attn_mask=None):
        ...
        residual = self.fc_residual(q)

        k = self.fc_k(k).view(bz * self.n_heads, -1, self.dim_per_head)
        v = self.fc_v(v).view(bz * self.n_heads, -1, self.dim_per_head)
        q = self.fc_q(q).view(bz * self.n_heads, -1, self.dim_per_head)

        scale = (k.size(-1)) ** (0.5)
        context, attention = self.dot_product_attention(q, k, v, scale, attn_mask=attn_mask)
...
class HeterogeneousEncoderLayer(nn.Module):
    def __init__(self, in_dim, model_dim, num_heads, dropout=0.0):
        ...
        self.attention = MultiHeadAttention(in_dim, model_dim, num_heads, dropout)
        self.heter_attention = MultiHeadAttention(in_dim, model_dim, num_heads, dropout)
       

    def forward(self, inputs, attn_mask=None):
       # _, heter_attention = self.heter_attention(inputs, inputs, inputs, None)
        #heter_attention = F.gumbel_softmax(heter_attention, tau=1)
